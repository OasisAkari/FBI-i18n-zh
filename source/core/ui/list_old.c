...
typedef struct {
    ...
    u64 nextActionTime;
    void (*update)(ui_view* view, void* data, list_item_old** items, u32** itemCount, list_item_old* selected, bool selectedTouched);
    ...
    if(listData->items != NULL && listData->itemCount != NULL) {
        ...
        for(u32 i = 0; i < *listData->itemCount && y < y2; i++) {
            ...
            if(y > y1 - stringHeight) {
                ...
                screen_draw_string(listData->items[i].name, x, y, 0.5f, 0.5f, listData->items[i].color, false);
